...
$query = "select * from  sd_item_l WHERE activity = 1 AND top_pro = 1 ORDER BY id DESC";
...
$query="select id, item_name from sd_item_l WHERE activity = 1 AND top_pro = 1 order by id DESC
...
$HTML='';
							...
							if ($stmt = $mysqli->prepare("SELECT id, item_name, price, dollar, dollar_discount, discount_price, img1, discount_per
							 from sd_item_l WHERE activity = 1 AND top_pro = 1
								ORDER BY id DESC limit  ".mysqli_real_escape_string($mysqli,$start).",$recordsPerPage")){
							$stmt->execute();    // Execute the prepared query.
							// get variables from result.
							$stmt->bind_result($item_id, $item_name, $price, $dollar, $dollar_discount, $discount_price, $img1, $discount_per);
							$stmt->store_result();
							  }
									...
									<div class="product-info">
										<a href="product/'.$item_id.'/'.urlencode($str).'" class="product-name"><span>'.$item_name.'</span></a>';
													...
													<div class="mrb6">';
														...
														echo '
														<div class="wrap-price mrgn0">
															<ins><p class="product-price">US $'.number_format($discount_price / $currency ).'.00</p></ins> 
															<del><p class="product-price">US $'.number_format($price / $currency ).'.00</p></del>
														</div>';
														}
														else{
														...
														echo'
														 <div class="wrap-price mrgn0">
															<ins><p class="product-price">US $'.number_format($price / $currency).'.00 </p></ins> 
														</div>';
															}
														echo'
													</div>
