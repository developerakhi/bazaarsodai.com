...
$getID = $_SESSION['user_id'];
 if ($stmt = $mysqli->prepare("SELECT mobile
			FROM sd_client
				WHERE id = ?
			LIMIT 1")) {
        ...
        $stmt->bind_result($usr_mobile);
        $stmt->fetch();
		}
		
 $query = "select * from sd_client WHERE ref = '".$usr_mobile."' ORDER BY id DESC";
 $ppage = 'all_ref.php';
...
$query="select id from sd_client order by id DESC
...
?>	 
      ...
      <table class="table cart-table">
                                        <thead style="background-color: #ffc46b;">
                                            <tr>
                                                ...
                                                <th class="table-title">Name</th>
												 <th class="table-title">Mobile</th>
										...
										global $mysqli;		
										if ($stmt = $mysqli->prepare("SELECT id, name, l_name, mobile, email, date
											from sd_client 
												WHERE ref = '".$usr_mobile."'
											ORDER BY id DESC limit ".mysqli_real_escape_string($mysqli,$start).",$recordsPerPage")){
										$stmt->execute();    // Execute the prepared query.
										// get variables from result.
										$stmt->bind_result($id, $name, $l_name, $mobile, $email, $date);
										$stmt->store_result();
											}
										...
										?> 
                                            <tr>
                                                ...
                                                <td class="product-price"><?php echo $date;?></span></td>
                                                 <td class="product-code"><?php echo $name . ' ' .$l_name; ?></td>
                                                <td class="product-nam"><?php echo $mobile;?></td>

                                                <td class="product-code"><?php echo $email;?></td>
