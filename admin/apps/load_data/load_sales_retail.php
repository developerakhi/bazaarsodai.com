...
if ($search != ''){
$query = "select * from sd_order_more WHERE name = '".$search."' ";
}
elseif ($payment_method != ''){
$query = "select * from sd_order_more WHERE mathod = '".$payment_method."' ";
}
elseif ($payment_status != ''){
$query = "select * from sd_order_more WHERE payment_status = '".$payment_status."' ";
}
		  ...
		  <thead>
             <tr class="info_member">
					 ...
					 <?php 		
                        ...
                        WHERE order_id LIKE '%$search%' || customer_name LIKE '%$search%' || mobile LIKE '%$search%' || email LIKE '%$search%'
						...
						global $mysqli;
                        if ($stmt = $mysqli->prepare("SELECT id, order_id, customer_name, mobile, g_total, mathod, delivery_time, date, activity, payment_status from sd_order_more
						WHERE mathod = '".$payment_method."' ORDER BY id DESC limit ".mysqli_real_escape_string($mysqli,$start).",$recordsPerPage")){
                        $stmt->execute();   
                        $stmt->bind_result($id, $order_id, $customer_name, $mobile, $g_total, $mathod, $delivery_time, $date, $activity, $payment_status);
                        $stmt->store_result();
						}
						...
						global $mysqli;
                        if ($stmt = $mysqli->prepare("SELECT id, order_id, customer_name, mobile, g_total, mathod, delivery_time, date, activity, payment_status from sd_order_more   
						WHERE payment_status = '".$payment_status."' ORDER BY id DESC limit ".mysqli_real_escape_string($mysqli,$start).",$recordsPerPage")){
                        $stmt->execute();    
                        $stmt->bind_result($id, $order_id, $customer_name, $mobile, $g_total, $mathod, $delivery_time, $date, $activity, $payment_status);
                        $stmt->store_result();
						}
					...
					while ($stmt->fetch()) { 
    					...
    					$newdeliveryDateTime = date( 'j F Y H:i:s',strtotime($delivery_time) ) ;
