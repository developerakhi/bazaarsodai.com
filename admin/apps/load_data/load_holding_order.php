...
$query = "select * from sd_order_more WHERE activity = 3";

if ($search != ''){
$query = "select * from sd_order_more WHERE activity = 3 AND name = '".$search."' ";
}
elseif ($payment_method != ''){
$query = "select * from sd_order_more WHERE activity = 3 AND mathod = '".$payment_method."' ";
}
elseif ($payment_status != ''){
$query = "select * from sd_order_more WHERE activity = 3 AND payment_status = '".$payment_status."' ";
}
...
if($lastpage > 1)
    {   
        ...
        if ($page > 1)
            $pagination.= "<li><a href=\"holding_order#Page=".($prev)."\" onClick='changePagination(".($prev).");'>&laquo; Previous&nbsp;&nbsp;</a></li>";
        else
        ...
        {   
            for ($counter = 1; $counter <= $lastpage; $counter++)
            {
                if ($counter == $page)
                    ...
                    $pagination.= "<li><a href=\"holding_order#Page=".($counter)."\" onClick='changePagination(".($counter).");'>$counter</a></li>";     
        ...
        elseif($lastpage > 5 + ($adjacents * 2))
        {
            if($page < 1 + ($adjacents * 2))
            {
                ...
                $pagination.= "...";
                $pagination.= "<li><a href=\"holding_order#Page=".($lpm1)."\" onClick='changePagination(".($lpm1).");'>$lpm1</a></li>";
                $pagination.= "<li><a href=\"holding_order#Page=".($lastpage)."\" onClick='changePagination(".($lastpage).");'>$lastpage</a></li>";   
           
           }
           elseif($lastpage - ($adjacents * 2) > $page && $page > ($adjacents * 2))
           {
               $pagination.= "<li><a href=\"holding_order#Page=\"1\"\" onClick='changePagination(1);'>1</a></li>";
               $pagination.= "<li><a href=\"holding_order#Page=\"2\"\" onClick='changePagination(2);'>2</a></li>";
               $pagination.= "...";
		  ...
		  <thead>
             <tr class="info_member">
					 ...
					 <?php
                        ...
                        WHERE activity = 3 AND order_id LIKE '%$search%' || customer_name LIKE '%$search%' || mobile LIKE '%$search%' || email LIKE '%$search%'
						...
						global $mysqli;
                        if ($stmt = $mysqli->prepare("SELECT id, order_id, customer_name, mobile, g_total, mathod, delivery_time, date, activity, payment_status from sd_order_more
						WHERE activity = 3 AND mathod = '".$payment_method."' ORDER BY id DESC limit ".mysqli_real_escape_string($mysqli,$start).",$recordsPerPage")){
                        $stmt->execute();   
                        $stmt->bind_result($id, $order_id, $customer_name, $mobile, $g_total, $mathod, $delivery_time, $date, $activity, $payment_status);
                        $stmt->store_result();
						}
						...
						global $mysqli;
                        if ($stmt = $mysqli->prepare("SELECT id, order_id, customer_name, mobile, g_total, mathod, delivery_time, date, activity, payment_status from sd_order_more   
						WHERE activity = 3 AND payment_status = '".$payment_status."' ORDER BY id DESC limit ".mysqli_real_escape_string($mysqli,$start).",$recordsPerPage")){
                        $stmt->execute();    
                        $stmt->bind_result($id, $order_id, $customer_name, $mobile, $g_total, $mathod, $delivery_time, $date, $activity, $payment_status);
                        $stmt->store_result();
						}
                        ...
                        WHERE activity = 3 ORDER BY id DESC limit ".mysqli_real_escape_string($mysqli,$start).",$recordsPerPage")){
                        $stmt->execute(); 
