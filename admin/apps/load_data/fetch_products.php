	...
	<table class="table table-hover">
		  <thead>
            <tr class="info_member">
					 ...
					 <?php 		
                        global $mysqli;
                        if ($stmt = $mysqli->prepare("SELECT id, item_name, item_code, category, delivery_charge, price, img1 from sd_item_l
						WHERE category = ?
                            ORDER BY id DESC")){
                        $stmt->bind_param('s', $country_id);  // Bind "$email" to parameter.
                        $stmt->execute();    // Execute the prepared query.
                        // get variables from result.
                        $stmt->bind_result($item_id, $item_name, $item_code, $category, $delivery_charge, $sell, $img1);
                        $stmt->store_result();
