...
$query = "select * from sd_order_more WHERE activity = 1";

if ($search != ''){
$query = "select * from sd_order_more WHERE activity = 1 AND name = '".$search."' ";
}
elseif ($payment_method != ''){
$query = "select * from sd_order_more WHERE activity = 1 AND mathod = '".$payment_method."' ";
}
elseif ($payment_status != ''){
$query = "select * from sd_order_more WHERE activity = 1 AND payment_status = '".$payment_status."' ";
}
		  ...
		  <thead>
             <tr class="info_member">
					 ...
					 <?php 		
                        ...
                        WHERE activity = 1 AND order_id LIKE '%$search%' || customer_name LIKE '%$search%' || mobile LIKE '%$search%' || email LIKE '%$search%'
						...
						global $mysqli;
                        if ($stmt = $mysqli->prepare("SELECT id, order_id, customer_name, mobile, g_total, mathod, delivery_time, date, activity, payment_status from sd_order_more
						WHERE activity = 1 AND mathod = '".$payment_method."' ORDER BY id DESC limit ".mysqli_real_escape_string($mysqli,$start).",$recordsPerPage")){
                        $stmt->execute();   
                        $stmt->bind_result($id, $order_id, $customer_name, $mobile, $g_total, $mathod, $delivery_time, $date, $activity, $payment_status);
                        $stmt->store_result();
						}
						...
						global $mysqli;
                        if ($stmt = $mysqli->prepare("SELECT id, order_id, customer_name, mobile, g_total, mathod, delivery_time, date, activity, payment_status from sd_order_more   
						WHERE activity = 1 AND payment_status = '".$payment_status."' ORDER BY id DESC limit ".mysqli_real_escape_string($mysqli,$start).",$recordsPerPage")){
                        $stmt->execute();    
                        $stmt->bind_result($id, $order_id, $customer_name, $mobile, $g_total, $mathod, $delivery_time, $date, $activity, $payment_status);
                        $stmt->store_result();
						}
                        ...
                        WHERE activity = 1 ORDER BY id DESC limit ".mysqli_real_escape_string($mysqli,$start).",$recordsPerPage")){
                        $stmt->execute(); 
