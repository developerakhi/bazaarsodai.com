...
testing_loggin();
 	...
 	if (isset($_POST['cat'] , $_POST['item_name'] , $_POST['price'])) {
		...
		global $mysqli; 
        if ($insert_stmt = $mysqli->prepare("INSERT INTO sd_item_l (item_name, item_code, category, sub_cat, sub_sub, unit, quantity, sort_desc, details, video, price, discount_per, discount, discount_price, d_quantity, delivery_charge, s_offer, e_offer, img1, top_pro, popular, shop_id, date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")) {
            $insert_stmt->bind_param('sssssssssssssssssssssss', $title, $item_code,  $category, $sub_cat, $sub_sub, $unit, $quantity, $short_desc, $details, $video, $price, $discount_per, $discount, $discount_price, $d_quantity, $delivery_charge, $s_offer, $e_offer, $image1, $top_pro, $popular, $shop_id, $date);
