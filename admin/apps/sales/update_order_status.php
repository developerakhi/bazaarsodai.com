...
testing_loggin();
 	...
 	if (isset($_POST['order_id'])) {
		...
		if ($payment_status != NULL){	
			global $mysqli;
			if ($insert_stmt = $mysqli->prepare("UPDATE sd_order_more SET payment_status=? WHERE id = ? LIMIT 1")){
		...
		if ($delivery_status != NULL){	
			global $mysqli;
			if ($insert_stmt = $mysqli->prepare("UPDATE sd_order_more SET activity=? WHERE id = ? LIMIT 1")){
