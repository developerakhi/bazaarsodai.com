...
public function clr_data() 
	...
	if (isset($_GET['ItemID'])) { 
		...
		$url_string_cat = urlencode($cat_name);
			...
			global $mysqli;
			$stmt = $mysqli->prepare("SELECT menu_id, menu_name, meta_desc, meta_key, page_cn, img1, img2, type, activity FROM menu WHERE menu_id = ? LIMIT 1");
			$stmt->bind_param('s', $url_string_cat);  
			$stmt->execute();    
			$stmt->store_result();
			$stmt->bind_result($menu_id, $menu_name, $meta_desc, $meta_key, $page_cn, $img1, $img2, $type, $activity);
			$stmt->fetch();
			$stmt->close();
			if ($type == 1){ $type_selected1 = 'selected="selected"'; }  
			if ($type == 2){ $type_selected2 = 'selected="selected"'; }  
                         ...
                         <label for="exampleInputEmail1">Type</label>
                 			  <label>Select a position <span class="red_star">*</span></label>
                                 <select name="type" class="form-control" required="">
                                   	<option value="1" '.$type_selected1.'>All Position</option>
									<option value="2" '.$type_selected2.'>Left Menu</option>
                                  </select>
					</div>
	...
	echo 
		'
					...
					<div class="form-group col-md-12"></div>	
					 ...
					 <div class="form-group col-md-4">
 						...
 						<div class="form-group col-md-4">
						     ...
						     <input type="text" name="quantity" value="1" class="form-control" placeholder="" autocomplete="off">
						</div>
						
						<div class="form-group col-md-4">
							 <label>Unit</label>
						     <input type="text" name="unit" class="form-control" placeholder="KG/PC/LTR/ETC." autocomplete="off">
						</div>
				    ...
				    <div class="form-group col-md-3">
    					 ...
    					 <input type="text" name="delivery_charge" placeholder="Delivery Charge" class="form-control">
					</div>
					...
					<div class="form-group col-md-4"> 
                       <label>Special Product  <span class="red_star">*</span></label>
					       <label>
                    ...
                    <div class="form-group col-md-4"> 
                       <label>Latest Product  <span class="red_star">*</span></label>
					       <label>
                    ...
                    <div class="form-group col-md-4"> 
                       <label>
						   <input class="form-check-input" type="checkbox" name="s_offer" value="1" />
					   </label>&nbsp;
					   <label>Offers</label>&nbsp;&nbsp;
					   
					   
					    <label>
						   <input class="form-check-input" type="checkbox" name="get_discount" value="1" />
					    </label>
						<label>Discount</label>&nbsp;&nbsp;
...
public function update_product() 
	...
	if (isset($_GET['ItemID'])) { 
		...
		$url_string_item = urlencode($cat_name);
			...
			global $mysqli;
			$stmt = $mysqli->prepare("SELECT id, shop_id, item_name, item_code, sub_cat, sub_sub, category, sort_desc, details, video, img1, img2, discount_per, discount, discount_price, d_quantity, delivery_charge, s_offer, free_delivery, get_discount, e_offer, price, top_pro, popular, deal, date, up_date, quantity, unit, activity 
			FROM sd_item_l
			  WHERE id = ?
				LIMIT 1");
			...
			// get variables from result.
			$stmt->store_result();
			$stmt->bind_result($id, $shop_id, $item_name, $item_code, $sub_cat, $sub_sub, $category, $sort_description, $details, $video, $img12, $img2, $discount_per, $discount, $discount_price, $d_quantity, $delivery_charge, $s_offer, $free_delivery, $get_discount, $e_offer, $sell, $top_pro, $popular, $deal, $date, $up_date, $quantity, $unit, $activity);
			$stmt->fetch();
			$stmt->close();
		...
		<div class="form-group">
            <input type="hidden" name="pro_id" placeholder="Item name" value="'.$id.'" class="form-control" required="required">
                        ...
                        <div class="form-group col-md-8">
    						<label>Delivery Charge</label>
    						<input type="text" name="delivery_charge" value="'.$delivery_charge.'" class="form-control">
    					</div>
                    ...
                    <div class="form-group col-md-8">
						...
						<div class="form-group col-md-4">
                           <label>Quantity </label>
                           <input type="text" name="quantity" placeholder="" value="'.$quantity.'" class="form-control">
                       </div>
					
						<div class="form-group col-md-4">
                           <label>Unit </label>
                           <input type="text" name="unit" placeholder="KG/PC/LTR/ETC." value="'.$unit.'" class="form-control">
                       </div>
                    ...
                    <div class="form-group col-md-4"> 
					   ...
					   <label>
						   <input class="form-check-input" type="checkbox" name="s_offer" value="1" '; if($s_offer == 1) {echo 'checked="checked"';} echo ' />
					   </label>&nbsp;
					   <label>Offers</label>&nbsp;&nbsp;
					   
					   
					    <label>
						   <input class="form-check-input" type="checkbox" name="get_discount" value="1" '; if($get_discount == 1) {echo 'checked="checked"';} echo ' />
					    </label>
						<label>Discount</label>&nbsp;&nbsp;
						
					    <label>
						   <input class="form-check-input" type="checkbox" name="free_delivery" value="1" '; if($free_delivery == 1) {echo 'checked="checked"';} echo ' />
					    </label>&nbsp;&nbsp;
					    <label>Free Delivery</label>
                    </div>
