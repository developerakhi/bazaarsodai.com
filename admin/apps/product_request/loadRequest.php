...
if ($search != ''){
$query = "select * from product_request WHERE name = '".$search."' ";
}	
...
if($lastpage > 1)
    {   
        ...
        if ($page > 1)
            ...
            $pagination.= "<li class='previous disabled'><a href='all_request'>&laquo; Previous&nbsp;&nbsp;</a></li>";   
        ...
        if($page < $counter - 1)
            ...
            $pagination.= "<li class='previous disabled'><a href='all_request'>Next &raquo;</a></li>";
...
<div class="box-body table-responsive no-padding">
	<table class="table table-hover">
		...
		<tbody>
			...
			if ($search != ''){
			global $mysqli;
			$catstm = $mysqli->prepare("SELECT id, name, phone, address, product_name, image from product_request WHERE name LIKE '%$search%' || product_name LIKE '%$search%' || phone LIKE '%$search%'
			ORDER BY id ASC limit ".mysqli_real_escape_string($mysqli,$start).",$recordsPerPage");
			$catstm->execute(); 
			$catstm->bind_result($id, $name, $phone, $address, $product_name, $image);
			
			}else{
				
			global $mysqli;
			$catstm = $mysqli->prepare("SELECT id, name, phone, address, product_name, image from product_request
			ORDER BY id ASC limit ".mysqli_real_escape_string($mysqli,$start).",$recordsPerPage");
			$catstm->execute(); 
			$catstm->bind_result($id, $name, $phone, $address, $product_name, $image);
			}
			...
			<tr>
				...
				<td><img class="req_pro" data-image-id="<?php echo $id; ?>" src="../images/products/<?php echo $image; ?>" ></td>
