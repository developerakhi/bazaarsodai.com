...
$query = "select * from sd_client WHERE type = 1 AND customer_id = '".$inID."' ";
}

elseif ($userID != ''){
$query = "select * from sd_client WHERE type = 1 AND id = '".$userID."' ";
}
else 
{
	...
	<table class="table table-hover">
		  <thead>
             <tr class="info_member">
					 ...
					 if ($inID != ''){
                        global $mysqli;
                        if ($stmt = $mysqli->prepare("SELECT id, item_code, stall_name, name, mobile, email, address, activity, date 
							from sd_client
								WHERE type = 1 AND customer_id = '".$inID."'
                            ORDER BY id DESC limit ".mysqli_real_escape_string($mysqli,$start).",$recordsPerPage")){
                        $stmt->execute();    // Execute the prepared query.
                        // get variables from result.
                        $stmt->bind_result($client_id, $item_code, $stall_name, $name, $mobile, $email, $address, $type, $date);
                        $stmt->store_result();
                          }
					 }
					 elseif ($userID != ''){
                        ...
                        global $mysqli;
                        if ($stmt = $mysqli->prepare("SELECT id, item_code, stall_name, name, mobile, email, address, location, activity, date 
							from sd_client
								WHERE type = 1 AND id = '".$userID."'
                            ORDER BY id DESC limit ".mysqli_real_escape_string($mysqli,$start).",$recordsPerPage")){
                        $stmt->execute();    // Execute the prepared query.
                        // get variables from result.
                        $stmt->bind_result($client_id, $item_code, $stall_name, $name, $mobile, $email, $address, $location, $type, $date);
                        $stmt->store_result();
                          }
					 }
					 else
					 {
						global $mysqli;
                        if ($stmt = $mysqli->prepare("SELECT id, name, l_name, mobile, email, address, location, activity, date 
							from sd_client
								WHERE type = 1
                            ORDER BY id DESC limit ".mysqli_real_escape_string($mysqli,$start).",$recordsPerPage")){
                        $stmt->execute();    // Execute the prepared query.
                        // get variables from result.
                        $stmt->bind_result($client_id, $name, $l_name, $mobile, $email, $address, $location, $type, $date);
                        $stmt->store_result();
                          }
					 }
                         while ($stmt->fetch()) { 
                                ...
                                <td style="text-align: center;">
                                   <div class="btn-group">
                                      ...
                                      <ul class="dropdown-menu" role="menu" style="left: -84px;">
                                        <li><a href="update_client/<?php echo $client_id; ?>"><i class="fa fa-pencil"></i>Edit</a></li>
                                        <li><a href="apps/bin_cat/delete_client.php?actionID=view_all_clients&ClientID=<?php echo $client_id; ?>" title="Delete This Row" onClick="return confirm('Are you sure to Delete this customer?')"><i class="fa fa-close"></i>Delete This Row</a></li>
                                      </ul>
                                    </div>
