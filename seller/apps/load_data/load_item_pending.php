...
if ($inID != ''){
$query = "select * from sd_item_l WHERE activity = 2 AND item_code = '".$inID."' ORDER BY id DESC";
}

elseif ($userID != ''){
$query = "select * from sd_item_l WHERE activity = 2 AND id = '".$userID."' ORDER BY id DESC";
}

else 
{
$query = "select * from sd_item_l WHERE activity = 2 AND shop_id = '".$getID."' ORDER BY id DESC";
}
...
$query="select id, item_name, item_code from sd_item_l WHERE activity = 2 order by id DESC
...
$query="select id from sd_item_l WHERE activity = 2 order by id DESC";
//echo $query;
					 ...
					 <?php 		
						...
						if ($inID != ''){
							if ($stmt = $mysqli->prepare("SELECT id, item_name, item_code, category, price, img1 from sd_item_l
								  WHERE activity = 2 AND item_code = ?
                         ...
                         while ($stmt->fetch()) { 
                            ...
                            <tr>
                                ...
                                <td style="text-align: center;">
                                 ...
                                 <a href="apps/bin_cat/delete_item_pending.php?itemID=<?php echo $item_id; ?>" class="btn btn-danger btn-raised btn-xs" data-toggle="tooltip" data-placement="top" title="Delete This Row" onClick="return confirm('Are you sure to Delete this Item?')"><i class="fa fa-close"></i><div class="ripple-container"></div></a>
