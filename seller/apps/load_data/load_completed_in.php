...
if ($date != ''){
$query = "select * from sd_order_list WHERE activity = 1 AND date = '".$date."' ORDER BY id DESC";
}

elseif ($inID != ''){
$query = "select * from sd_order_list WHERE activity = 1 AND orderID = '".$inID."' ORDER BY id DESC";
}

else 
{
$query = "select * from sd_order_list WHERE activity = 1 ORDER BY id DESC";
}
	...
	<table class="table table-hover">
		  <thead>
					...
					elseif ($inID != ''){
						global $mysqli;
                        if ($stmt = $mysqli->prepare("SELECT id, orderID, shop_id, title, img, line_total, date from sd_order_list 
								WHERE activity = 1 AND orderID ='".$inID."'
                            ORDER BY id DESC limit ".mysqli_real_escape_string($mysqli,$start).",$recordsPerPage")){
                        $stmt->execute();    // Execute the prepared query.
                        // get variables from result.
                        $stmt->bind_result($id, $order_id, $shop_id, $title, $img, $line_total, $date);
