...
if ($inID != ''){
$query = "select * from sd_item_l WHERE activity = 1 AND item_code = '".$inID."' ORDER BY id DESC";
}

elseif ($userID != ''){
$query = "select * from sd_item_l WHERE activity = 1 AND id = '".$userID."' ORDER BY id DESC";
}

else 
{
$query = "select * from sd_item_l WHERE activity = 1 AND shop_id = '".$getID."' ORDER BY id DESC";
}
...
$query="select id, item_name, item_code from sd_item_l WHERE activity = 1 order by id DESC
...
$query="select id from sd_item_l WHERE activity = 1 order by id DESC";
//echo $query;
					 ...
					 <?php 		
						...
						if ($inID != ''){
							if ($stmt = $mysqli->prepare("SELECT id, item_name, item_code, category, price, img1 from sd_item_l
								  WHERE activity = 1 AND item_code = ?
