<?php
define("APP_ROOT", dirname(dirname(__FILE__)));
define("PRIVATE_PATH", APP_ROOT . "");
require_once(PRIVATE_PATH . "/functions/functions.php");
testing_loggin();

 	if (isset($_GET['ItemID'])) {
	
		// Sanitize and validate the data passed in
		$ItemID = filter_input(INPUT_GET, 'ItemID', FILTER_SANITIZE_NUMBER_INT);
		$shop_id = filter_input(INPUT_GET, 'shop_id', FILTER_SANITIZE_NUMBER_INT);

        // Delete From database 
		global $mysqli;
		if ($insert_stmt = $mysqli->prepare("DELETE FROM sd_order_list WHERE orderID =? AND shop_id	= ?")){
		$insert_stmt->bind_param('ss', $ItemID, $shop_id);
		
		if (!$insert_stmt->execute()) {
					header('Location: ../error.php?err=Registration failure: INSERT');
			   }
		 	 header('Location: ../../view_all_retail_invoices/9654526/delete_success');
		   }
		}
?>