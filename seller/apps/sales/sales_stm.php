...
public function company_details() 
	...
	$stmt = $mysqli->prepare("SELECT id, shop_name, name, address, mobile, email from sd_merchant
	WHERE id = '".$getID."'	ORDER BY id ASC LIMIT 1 ");
	$stmt->execute();    // Execute the prepared query.
			// get variables from result.
	$stmt->store_result();
    $stmt->bind_result($client_id, $shop_name, $name, $address, $mobile, $email);
	$stmt->fetch();
	$stmt->close();
	...
	echo '<table width="100%" border="0" cellspacing="5" cellpadding="5">
          <tr>
			...
			<h1 style="margin:0px;font-size: 35px; ">&nbsp;'.$shop_name.'</h1>
				Phone : '.$mobile.'
