...
testing_loggin();
 	if (isset($_POST['name'])) {
		...
		global $mysqli;
		if ($insert_stmt = $mysqli->prepare("UPDATE sd_client SET  name=?, l_name=?, mobile=?, email=?, address=?, activity=? WHERE id = ? LIMIT 1")){
		// Bind the variables:
		$insert_stmt->bind_param('ssssssi',  $name, $l_name, $mobile, $email, $address, $activity, $id);
		// Execute the query:
